...
class Repository {
  ...
  getTournaments(cb) {
    this.db
      .find({})
      .sort({ createdAt: -1 })
      .toArray(function(err, tournaments) {
        cb(err, tournaments);
      });
  }
  ...
  updateGame(tournamentId, game, cb) {
    ...
    if (game.isFinale) {
      ...
      if (game.games.length == 2) {
        if (
          (game.games[0].team1Score > game.games[0].team2Score &&
            game.games[1].team1Score < game.games[1].team2Score) ||
          (game.games[0].team1Score < game.games[0].team2Score &&
            game.games[1].team1Score > game.games[1].team2Score)
        ) {
          game.games.push({});
  ...
  playerRanking(tournament) {
    ...
    const games = _.filter(tournament.games, function(game) {
      return !game.isFinale;
    });
  ...
  startTournament(tournamentId, cb) {
    ...
    this.getTournament(tournamentId, function(err, tournament) {
      ...
      try {
        ...
        const optimazedValue =
          tournament.players.length * gamesPerPlayer * 10000 +
          tournament.players.length * gamesPerPlayer * 2;
  ...
  generateGames(tournament) {
    assert(
      ...
      "Tournament needs atleast four players"
    );
    assert(
      (tournament.gamesPerPlayer * tournament.players.length) % 4 == 0,
      "Number of games per player multiplied by number of players needs to divisable with four"
    );
    ...
    function pickP3(p1, p2, playerpool, games) {
      ...
      for (var i = 0; i < playerPool.length; i++) {
        ...
        const playedWith =
          _playedWith(p1, candidate, games) + _playedWith(p2, candidate, games);
        const playedAgains =
          _playedAgainst(p1, candidate, games) +
          _playedAgainst(p2, candidate, games);
        const gameCount = _gameCount(candidate, games);
