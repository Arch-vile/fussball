...
fussBallApp.controller("changeLogController", [
  "$scope",
  "$http",
  function($scope, $http) {
    $scope.loaded = false;
    $http
      .get(
        "https://api.github.com/repos/arch-vile/fussball/commits?per_page=50"
      )
      .then(function(response) {
        $scope.loaded = true;
    ...
    $scope.commitFilter = function(commit) {
      return (
        ...
        commit.commit.message.toLowerCase().includes("bugfix")
      );
    };
  }
]);

fussBallApp.controller("signupController", [
  "$scope",
  "$http",
  function($scope, $http) {
    $scope.loaded = false;
...
fussBallApp.controller("gameController", [
  "$scope",
  "$http",
  function($scope, $http) {
    $scope.loaded = false;
    ...
    $scope.submitScore = function(game) {
      $http
        .put(
          "/tournaments/" + tournamentId + "/games/" + game.id,
          game,
          headers()
        )
        .then(function(response) {
...
function loadGames(scope, http) {
  http.get("/tournaments/" + tournamentId, headers()).then(function(response) {
    scope.loaded = true;
    scope.finale = _.filter(response.data.games, function(game) {
